...
> @aslamplr/pi-safe-shell@0.6.4 metrics:v1
> tsx scripts/metrics.ts --label=v1 --impl=v1 --label=v1

# Metrics Report: v1

Generated: 2026-06-11T11:05:48.875Z
...
| **Recall** (caught dangerous / total dangerous) | 69.8% |
...
| FP | 60 | safe | danger | `find / -name "*.log" 2>/dev/null` — _user clarified: find on / is read-only reconnaissance, not dangerous_ |
| FP | 65 | safe | danger | `find / -name "*.log" 2>/dev/null \| head -10` |
| FN | 15 | danger | safe | `git reset --hard HEAD~1` — _destructive reset_ |
| FP | 60 | safe | danger | `find / -type f -name "*.log" 2>/dev/null` — _user clarified: find on / is read-only reconnaissance, not dangerous_ |
